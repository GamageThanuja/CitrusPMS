...
import { getHotelRoomTypeImages } from "@/controllers/hotelRoomTypeImageController";
...
export default function GalleryTab() {
  ...
  const [activeTab, setActiveTab] = useState<"property" | "room" | "item">(
    "property"
  );
  ...
  const fetchImages = async () => {
    try {
      // Retrieve tokens and hotel data from localStorage
      ...
      // Use Promise.all to parallel fetch both APIs
  ...
  const imagesToDisplay = [...hotelImages, ...roomImages].filter((img) => {
    const desc = img.description?.trim().toLowerCase() || "";

    if (activeTab === "room") return desc.startsWith("room");
    if (activeTab === "item") return desc.startsWith("item");

    return !desc.startsWith("room") && !desc.startsWith("item");
  });
