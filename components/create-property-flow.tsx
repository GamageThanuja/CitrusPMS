...
import { updateHotelStripePayment } from "@/redux/slices/stripePaymentSlice";
...
export default function CreatePropertyFlow() {
  ...
  const stripeUpdateSentRef = useRef(false);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        await pushStripePaymentUpdate({
          stripePaymentRef: data?.id ?? sid, // checkout_session.id
          stripeAuthCode: data?.payment_intent ?? data?.pi ?? null, // PI if you return it
        });

        // ⛔️ DO NOT create here.
        // If a hotel was already created earlier, just jump user to images step.
  ...
  useEffect(() => {
    ...
    if (sessionId && hasAlreadyPostedHotel()) {
      // hotel is already created from the plan step — just move on
      setCurrentStep((s) => Math.min(s + 1, steps.length - 1));
    }
  }, [search]);

  async function pushStripePaymentUpdate(opts: {
    stripePaymentRef?: string | null; // e.g., checkout_session.id
    stripeAuthCode?: string | null; // e.g., payment_intent id
  }) {
    if (stripeUpdateSentRef.current) return;
    stripeUpdateSentRef.current = true;

    try {
      await (dispatch as any)(
        updateHotelStripePayment({
          // hotelId omitted → slice will read from localStorage.selectedProperty.id
          stripePaymentRef: opts.stripePaymentRef ?? "unknown_session",
          isPaymentTrue: true,
          stripeAuthCode: opts.stripeAuthCode ?? null,
          authTimeStamp: new Date().toISOString(),
        })
      ).unwrap();
    } catch (e) {
      // don’t block UX — just log
      console.error("Stripe payment PUT failed:", e);
    }
  }
...
<body style="margin:0; padding:0; background-color:#f8fafc;">
  ...
  // add this helper somewhere near other utils
  function hasAlreadyPostedHotel(): boolean {
    return (
      hasPostedHotel || localStorage.getItem("hotelmate:hasPostedHotel") === "1"
    );
  }
  ...
  const createHotelNow = async () => {
    if (hasAlreadyPostedHotel()) return null;
    ...
    try {
      ...
      localStorage.setItem("hotelmate:hasPostedHotel", "1");
              ...
              <EmbeddedCheckoutProvider
                ...
                options={{
                  ...
                  onComplete: async () => {
                    ...
                    try {
                      // If your API supports looking up by client_secret:
                      const res = await fetch(
                        `/api/stripe/checkout-session?client_secret=${encodeURIComponent(
                          checkoutClientSecret!
                        )}`
                      );
                      const data = await res.json();

                      // Fallback to just sending what we have if lookup isn’t available
                      await pushStripePaymentUpdate({
                        stripePaymentRef: data?.id ?? null,
                        stripeAuthCode: data?.payment_intent ?? null,
                      });
                    } catch (e) {
                      // still attempt a minimal update so your BE marks it paid
                      await pushStripePaymentUpdate({
  ...
  const nextStep = async () => {
    ...
    if (
      title.includes("choose your subscription") &&
      !hasAlreadyPostedHotel()
    ) {
      const created = await createHotelNow();
      if (!created) return; // stop navigation if creation failed
    }
