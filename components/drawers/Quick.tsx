...
import { Trash2, Calendar, Youtube, Plus, Loader2 } from "lucide-react";
import {
  Table,
  TableHeader,
  TableRow,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Step 1: Fetch reservation full details
        ...
        if (fetchReservationById.fulfilled.match(reservationResult)) {
          ...
          onCreated?.();
          try {
            const toEmail = recipientEmail(); // localStorage("email") → "rememberedEmail" → guestEmail
            ...
            if (toEmail) {
              ...
              const emailRooms = rooms.map((r) => {
                const rt = roomTypes.find(
                  (x) => String(x.hotelRoomTypeID) === String(r.roomType)
                );

                const daysVals = Object.values(r.daysMap || {}).map((v: any) =>
                  Number(v || 0)
                );

                const total = daysVals.reduce((s, v) => s + v, 0);

                const avg = daysVals.length
                  ? total / daysVals.length
                  : Number(r.rate || 0);
              ...
              const html = buildEmailHtml({
                ...
                currency: currency || hotelObj?.hotelCurrency || "USD",
              ...
              await (dispatch as any)(
                sendCustomEmail({
                  ...
                  // ccEmails: getLS("ccEmailsCsv"),

                  // bccEmails: getLS("bccEmailsCsv"),

                  // replyToEmail: getLS("replyTo") || guestEmail,
              ...
              console.warn(
                "No email in localStorage('email' | 'rememberedEmail') or guestEmail — skipping sendCustomEmail."
              );
            }
          } catch (e) {
          ...
          <ScrollArea className="flex-1 overflow-y-auto py-4  ">
            <form className="space-y-4 mx-2">
              <div className="bg-muted/50 dark:bg-slate-900 py-2 rounded-md ">
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-4 bg-muted/50 dark:bg-slate-900 py-2 rounded-md ">
                  <div>
    ...
    ${
      reservationType === "tentative"
        ...
        : "sm:grid-cols-4 md:grid-cols-4 lg:grid-cols-4"
              ...
              {/* rate section */}
              <div className="bg-muted/50 dark:bg-slate-900 py-2 rounded-md  items-center">
                  ...
                  <TableBody>
                    {rooms.map((row, idx) => (
                      <TableRow key={idx}>
                        ...
                        <TableCell>
                          <Select
                            value={selectOccupancy}
                            onValueChange={setSelectOccupancy}
                          >
                            <SelectTrigger>
                        ...
                        <TableCell>
                          <Select
                            value={selectBed}
                            onValueChange={setSelectBed}
                          >
                            <SelectTrigger>
                              <SelectValue placeholder="Bed Type" />
                            </SelectTrigger>
                            <SelectContent>
                              <SelectItem value="doube">Doube</SelectItem>
                        ...
                        <TableCell>
                          <div className="relative w-28">
                            <Input
                              ...
                              value={row.is_foc ? "0" : String(row.rate ?? 0)}
                              onChange={(e) =>
              ...
              <div className="grid  grid-cols-4 gap-4 bg-muted/50 dark:bg-slate-900 py-2 rounded-md ">
                <div className="col-span-1 ">
                  <label className="text-sm font-medium block mb-1">
                    Title
                  </label>
                  ...
                  {guestName && showNameSuggestions && (
                    <SuggestionTable
                      ...
                      onSelect={(guest) => {
                        ...
                        setSelectedGuestProfileId(guest.guestProfileId ?? null); // 👈 capture ID
                        setRepeatGuest(true); // 👈 repeat
