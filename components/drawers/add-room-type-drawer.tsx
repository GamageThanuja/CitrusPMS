...
import { RoomTypeFormData as EditRoomTypeFormData } from "./edit-room-type-drawer";
...
export function AddRoomTypeDrawer({
  ...
  const ensureRevenueAccountForRoomType = async (roomTypeName: string) => {
    const accountCode = `REV-${roomTypeName}`
      .toUpperCase()
      .replace(/[^A-Z0-9]+/g, "_")
      .slice(0, 30);
    ...
    const glPayload = {
      ...
      finAct: false, // create active
    };

    console.log("GL create payload:", glPayload);
    const created = await dispatch(createGlAccount(glPayload)).unwrap();
    return created.accountID as number;
  };
  ...
  const handleCreateRoomType = async (): Promise<number | null> => {
    ...
    // 2) Create GL revenue account first, get its accountID
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const features = [
        ...(Array.isArray(formData.amenities) ? formData.amenities : []),
        ...(Array.isArray(formData.bedding) ? formData.bedding : []), // ⬅️ collect all beddings
        ...(formData.view ? [formData.view] : []),
        ...(Array.isArray(formData.washroom) ? formData.washroom : []),
      ].filter(
        (val): val is string => typeof val === "string" && val.trim() !== ""
      ...
      const submittedData: EditRoomTypeFormData = {
        ...
        bedding: Array.isArray(formData.bedding)
                          ...
                          <div key={category} className="mb-6">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-2">
                              {features.map((feature) => {
                                const isRadioType = category === "View";
                                const fieldName =
                                  category === "Amenities"
                                    ? "amenities"
                                    : category === "Bedding"
