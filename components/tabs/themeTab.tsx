...
export default function ThemeTab() {
  ...
  useEffect(() => {
    ...
    if (savedAutoTheme) {
      const systemTheme = window.matchMedia("(prefers-color-scheme: dark)")
        ...
        ? "dark"
        : "light";
      if (savedTheme !== systemTheme) applyTheme(systemTheme);
    ...
    if (savedColors) {
      try {
        const parsed = JSON.parse(savedColors) as Record<number, string>;
        setReservationStatuses((prev) =>
          prev.map((s) => ({ ...s, color: parsed[s.id] ?? s.color }))
        );
      } catch (err) {
        console.error("Error parsing reservation colors:", err);
      }
    }
  }, []);

  const applyTheme = (newTheme: "light" | "dark" | "system") => {
    // system -> detect once, then apply
    const resolved =
      newTheme === "system"
  ...
  const saveReservationColors = () => {
    const colorMap = reservationStatuses.reduce((acc, s) => {
      acc[s.id] = s.color;
      return acc;
  ...
  const resetReservationColors = () => {
    ...
    setReservationStatuses(defaults);
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {!autoTheme && (
            <RadioGroup
              value={theme}
              onValueChange={(val) => applyTheme(val as any)}
            >
              <div className="flex items-center space-x-2">
            ...
            {accentColors.map((c) => (
              <button
                ...
                className={`flex items-center gap-3 p-3 rounded-lg border-2 transition-colors ${
                  accentColor === c.value
                    ? "border-cyan-600 bg-gray-50 dark:bg-black"
                    : "border-gray-200 hover:border-gray-300"
                }`}
              >
                <div className={`w-6 h-6 rounded-full ${c.swatch}`} />
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Tag className="h-5 w-5" /> Reservation Status Colors
          </CardTitle>
          <CardDescription>
